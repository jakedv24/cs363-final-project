...
<%
    ...
    class QueryParam {
        ...
        int setStatementParameter(int index, PreparedStatement stmt) throws SQLException {
        	int ind = index;
        	switch (type) {
                case QueryParamType.HASHTAG_LIST:
                case QueryParamType.STATE_LIST:
                ...
                case QueryParamType.YEAR:
                case QueryParamType.YEAR_MULTI:
                    stmt.setInt(ind++, toNumber());
                    break;
                case QueryParamType.SUB_CATEGORY:
                case QueryParamType.HASHTAG:
                default:
                    stmt.setString(ind++, value);
                    break;
            }
            return ind;
        }
    ...
    Query q6 = new Query("Q6",
		...
		"WHERE t.tweeted_by=u.sname AND t.id=th.tweet_id AND h.hashtag IN (?)  AND u.belongs != 'na' " +
		"GROUP BY (u.sname) HAVING COUNT(DISTINCT h.hashtag) = ? " +
    ...
    q10.parameters.add(new QueryParam("year", "Year", QueryParamType.YEAR));
    q10.parameters.add(new QueryParam("states", "States", QueryParamType.STATE_LIST));
    ...
    q23.parameters.add(new QueryParam("k", "Number most used hashtags", QueryParamType.NUMBER));
